...
class Process extends BaseProcess
{
     ...
     */
    public function send (array $recordSet, ?callable $retryCallable = null): array
    {
        ...
        foreach ($sendData as $brokerId => $topicList) {
            ...
            $this->do($params, $client, $requiredAck, $config->getRetryTimes(), $config->getAutoRetry(), $retryCallable);
     ...
     * @param bool          $isAutoRetry
     * @param callable|null $retryCallable
     * @throws Exception
     */
    public function do ($params, $client, $requiredAck, int $retryTimes, bool $isAutoRetry, ?callable $retryCallable)
    {
        try {
            ...
            if ($isAutoRetry) {
                $retryTimes--;
                if ($retryTimes >= 0) {
                    $this->do($params, $client, $requiredAck, $retryTimes, $isAutoRetry, $retryCallable);
                }
            } else {
                if (is_callable($retryCallable)) {
                     ...
                     */
                    call_user_func($retryCallable, $exception->getMessage());
                }
            }
